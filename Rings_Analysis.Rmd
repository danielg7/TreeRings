...
```{r ACF_check}
for(k in unique(detrendedDF$Sample))
  {
  individual <- subset(detrendedDF,Sample == k)
  print(acf(x = individual$RingWidth,plot = TRUE,main = paste(unique(individual$Species),unique(individual$Sample),unique(individual$Site))))
  }
